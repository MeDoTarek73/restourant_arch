let service = new(require('../usecases/users'))();
...
module.exports = function(app, express) {
    ...
    router.get('/filter/:cuisine', auth, (req, res) => {
        service.filterByCuisines(req.params.cuisine).then(_res => {
            res.status(_res.status).send(_res.data);
        }).catch(_err => {
            res.status(_err.status).send(_err.message);
        });
    ...
    router.post('/filter', auth, (req, res) => {
        service.filter(req.body).then(_res => {
            res.status(_res.status).send(_res.data);
        }).catch(_err => {
            res.status(_err.status).send(_err.message);
        });
    ...
    router.post('/login', (req, res) => {
        service.login(req.body).then(_res => {
            res.status(_res.status).send(_res.data);
        }).catch(_err => {
            res.status(_err.status).send(_err.message);
        });
